...
1. HTTP 狀態碼的分類（1xx, 2xx, 3xx, 4xx, 5xx 各代表什麼）
   ...
   3xx：重新導向 - 表示需要進一步操作才能完成請求
   4xx：用戶端錯誤 - 表示請求有語法錯誤或無法完成(如404：找不到網頁或401：未授權等)
   5xx：伺服器錯誤 - 表示伺服器在處理請求時發生錯誤或伺服器端壞掉了，無法處理正確的請求(如500：內部伺服器錯誤等)

2. GET、POST、PATCH、PUT、DELETE 的差異
   答：
    GET：讀取資料。用於單純從伺服器獲取資料，不會修改伺服器上的資源(如抓取產品列表或購物車內容等)
    POST：新增資料。用於向伺服器提交資料以創建新的資料(如加入商品到購物車等、送出訂單等)
    PATCH：更新部分資料。用於更新伺服器上的資源的部分內容
    PUT：更新全部資料。用於更新伺服器上的資源的全部內容
    DELETE：刪除資料。用於從伺服器上刪除資源

3. 什麼是 RESTful API？
   答：一種設計風格，使開發者可以透過簡單且一致的方式來設計和使用 API。
   RESTful API 通常使用 HTTP 方法（GET、POST、PATCH、DELETE 等）來對資源進行操作，並且資源通常以 URL 的形式表示。
    RESTful API 的特點包括：
    資源路徑化 (URI)： 使用名詞來定義資源（例如 /products 而不是 /getProducts）。
    動詞明確化： 搭配上述的 HTTP 方法（GET, POST 等）來決定操作動作。
    狀態無關性： 伺服器不會紀錄客戶端的狀態，每個請求都包含所有必要的資訊。
